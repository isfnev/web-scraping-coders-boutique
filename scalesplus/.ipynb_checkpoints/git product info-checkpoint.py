                        ...
                        if next_tag and next_tag.name == 'ul':
                            ...
                            if tag.find('a'):
                                for i, a in enumerate(tag.find_all('a'), 1):
                                    dict[f'document link {i}'] = a.get('href')
                            elif tag.strong:
                                pass
                            else:
                                dict['description paragraph {para}'] = tag.text
                                para += 1
                player = soup.find(id='player')
                if player:
                    dict['Product video'] = player.get('data-src')[2:]
...
def main():
    urls_path = 'scalesplus/textfiles/product links.txt'
    df_store_path = 'scalesplus/textfiles/output1.csv'
    # link_index = 2
