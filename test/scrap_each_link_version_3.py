...
def return_link_center_stone_shape(soup):
    ...
    return link_center_stone_shape
...
def for_third_loop(link_material):
    headers = {
    ...
    if r.status_code == 200:
        ...
        with open('textfiles/extracted_version_3.csv','a',newline='') as file:
...
def for_second_loop(link_shape):
    headers = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML,                  like Gecko) Chrome/85.0.4183.83 Safari/537.36'
}
    ...
    link_carat_material = return_link_carat_material(soup)
    with concurrent.futures.ThreadPoolExecutor(max_workers=70) as executor:
        executor.map(for_third_loop, link_carat_material)
...
def for_first_loop(link_weight):
    ...
    link_center_stone_shape = return_link_center_stone_shape(soup)
    with concurrent.futures.ThreadPoolExecutor(max_workers=80) as executor:
        executor.map(for_second_loop, link_center_stone_shape)
