...
def return_url_in_chunks():
    ...
    return [(data[i:i+1000], i/1000) for i in range(12000, len(data), 1000)]
...
def give_url(link_info):
    ...
    try:
        ...
        with open('textfiles/exception.txt','a') as file:
            file.write(link+'\n')

def process_task(chunk_info):
    urls, chunk_index = chunk_info
    start = time()
    thread_takes = [(chunk_url, chunk_index) for chunk_url in urls]

    with concurrent.futures.ThreadPoolExecutor(max_workers=50) as executor:
         executor.map(give_url, thread_takes)
    print('Chunk no.', chunk_index,'takes',time()-start,'to execute')
...
if __name__=='__main__':
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=8) as executor:
        #  for chunk_url, chunk_index in return_url_in_chunks():
            # chunk_gen = return_url_in_chunks()
            # chunk_url, chunk_index = next(chunk_gen)

            # chunk_info = [(url, chunk_index) for url in chunk_url]
                                    executor.map(process_task, return_url_in_chunks())
    print(time()-start)
